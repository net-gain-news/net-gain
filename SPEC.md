...
- **Show notes are real HTML** — confirmed directly (2026-09-14) that Captivate's own show-notes editor is styled/rich text, not plain text: one paragraph per story, with the script's own "story names and links" section (when present) reproduced with each URL as a real, `nofollow`ed hyperlink, never altered.
- **Audio bitrate conversion before upload** (added 2026-09-21): Captivate requires 192kbps CBR, but the show's production software outputs 320kbps CBR — too high for Captivate to accept as-is. The pipeline transcodes a working copy to 192kbps CBR MP3 immediately before the Captivate upload specifically; the original higher-bitrate file stored in WordPress (used for website playback and as YouTube's video audio track) is left untouched, since neither of those destinations shares this constraint. Not tracked as its own dashboard step — an internal detail of the existing `captivate_published` step, surfaced through that step's own failure state if conversion ever fails.
...
- **URL structure** (revised 2026-09-18): bare top-level subdirectories (`studio-domain.com/{show-slug}/`) are canonical — no `/shows/` segment. This still consolidates every show's SEO authority under one domain/single WordPress install (the original rationale for subdirectories over subdomains, which still holds), it's just shorter and easier to say aloud for an audio show whose hosts verbally reference the URL. The original decision (`/shows/{show-slug}/`) was reversed before the site went live/indexed, so no redirects were needed — see the Net Gain News website-theming project's plan for the corresponding `class-website-rewrite.php` change and page re-parenting. Subdomains were considered and rejected as the default (only worth the complexity if a show might later spin off to a fully independent domain). **Custom domain mapping** is supported as a future/optional layer for a specific client — DNS/redirect pointing at the canonical subdirectory, not a separate infrastructure buildout.
...
- **Excerpt** (added 2026-09-14, after a live episode's page fell back to WordPress's own default — the full script concatenated — for lack of one): a dedicated `website_excerpt` field, generated by the same metadata-generation call as every other publish-facing field, drawing on the script's own opening lines rather than compressing the whole episode.
- **Story links are real, followed-safe hyperlinks**: the script's own "story names and links" section is linkified (WordPress core's `make_clickable()`) rather than left as inert plain-text URLs, with `rel="nofollow noopener"` added — these are outbound citations to third-party news sources, not endorsements this site should pass link equity to.
- **Episode list date**: each show's episode-list page shows the episode's actual **publish** date, not its recording date — the public post's own `post_date`, set at the moment it actually goes live (which in scheduled publish mode can be a day or more after recording, per Section 8.2's own example) — as its own element next to the title, not concatenated into the headline text.
...
- **SEO-optimized title, description, and tags** are a required, explicit project goal — generated by the same metadata-generation call as the Captivate/AIOSEO fields, following YouTube-specific conventions (first ~125 characters of description are what's visible before truncation; a real generated tag list). These must be written for genuine algorithmic discoverability (search matching and YouTube's own suggested-video algorithm), not merely human-readable correctness: the title leads with the episode's single most search/recommendation-relevant keyword or phrase rather than a generic show-name-first framing, and the description stays keyword-dense throughout rather than padded toward a target length — a shorter, denser description is explicitly preferred over a longer, diluted one, and should stay well under YouTube's 5000-character hard limit rather than aiming at it.
- **Ticker symbols for publicly-traded companies** (added 2026-09-12, per human-operator request): whenever an episode discusses a publicly-traded company, its stock ticker symbol (e.g. `$AAPL`) should be included at least once — in the YouTube description and as its own tag, and in the Captivate show notes and AIOSEO description — since investors commonly search by ticker in a way plain company names don't capture. This applies to every SEO-bearing field the shared metadata-generation call produces, not just YouTube's. Like the rest of this section's checklist, this is a generation-prompt instruction the model applies when relevant, not a mechanically checkable pre-publish criterion — there is no way to independently verify after the fact whether a given episode "should" have mentioned a ticker.
...
- **Concrete, story-specific imagery is a required goal, not a nice-to-have** (amended 2026-09-16, after the first three live episodes all produced generic conceptual imagery — anonymous people shaking hands, a vague chart glowing on a screen behind them — instead of anything tied to the lead story's actual specifics). The image-prompt generation call must identify one concrete, filmable detail genuinely present in the lead story and build the image around it, rather than illustrating the story's general topic or category; common stock-photo defaults (handshakes, anonymous meetings, vague dashboards) are explicitly disallowed unless the story is literally about that exact moment.
- **No company logos, ever** — an AI-drawn logo is known to render inaccurately (wrong colors, garbled wordmarks), and compositing genuine logo assets instead would require a maintained logo library or a logo-fetching service, judged not worth building yet. Logos are disallowed outright, with no per-story exception.
- **Financial iconography is allowed for money-related stories** (funding, valuation, revenue, a deal) — a currency symbol, a rising/falling arrow, a stock ticker, coins or banknotes — but must infer the currency that actually fits the story's context rather than defaulting to the US dollar sign; this show's stories are not all US-market-specific. Graphical only — never the amount spelled out as a word or numeral anywhere in the image (confirmed live, 2026-09-17: the word "BILLION" rendered as garbled background text for a $400M-pledge story before this was tightened).
...
- Recording days
- Target time (the earliest the tick loop will request today's script, not a deadline it must be ready by — actual availability follows shortly after, once the next tick loop pass picks it up)
